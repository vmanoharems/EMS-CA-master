...
CREATE PROCEDURE [dbo].[UpdateCRWSetEFC]   --  exec InsertUpdateCRW 1,2,61,2942,'610-05','9','ETC'
(
@BudgetID int,
@BudgetFileID int,
@DetailLevel int,
...
BEGIN
	...
	begin
	  ...
	  update EstimatedCostSet Set EFC=@SaveValue where COAID=@COAID and SetID=@SetID and BudgetId=@BudgetID and BudgetFileID=@BudgetFileID;
	...
	begin
		...
		--select @L1COA=COAID select * from BudgetCategoryFinal where cid=@L1P and BudgetId=@BudgetID and BudgetFileID=@BudgetFileID;
		--There is a bug in SQL that keeps returning the wrong COAID for the category. Using a dt to get to the proper category COAID
		select @L1P = CategoryID, @L1COA=BCF_COAID from 
			(select BAF.CategoryId,BCF.COAID as BCF_COAID,BAF.COAID from BudgetAccountsFinal BAF
			join BudgetCategoryFinal BCF
				on BAF.categoryID = BCF.cid and BAF.BudgetID = BCF.BudgetID and BAF.BudgetFileID = BCF.BudgetFileID
			where BAF.BudgetID = 1 and BAF.BudgetFileID=3
			group by BAF.CategoryID, BCF.COAID, BAF.COAID
		) dt where COAID = @COAID

		update EstimatedCost Set EFC= cast(EFC as int)+cast(@Changes as int) where
			COAID=@L1COA and BudgetId=@BudgetID and BudgetFileID=@BudgetFileID;
