...
BEGIN
	...
	if isnull(@COACode,'') = ''
		select @COACode = COACode from COA where DetailLevel =0 and ParentCode = ''
    ...
    if (@SegmentPosition = -1) -- Use -1 to return CASH accounts that are not in use by any other entity (bank, custodian, recipient)
		...
		If (@DT='Detail')
			...
			begin
			select A.AccountCode as  COANo,A.AccountCode as COACode,COAID,A.AccountCode  as COANo1  
			from COA  
			join tblAccounts A
			on COA.AccountID = A.AccountID
