...
CREATE PROCEDURE [dbo].[ProceedBudgetFinal]
(
@BudgetID int,
@BudgetFileID int,
@createdby int,
...
BEGIN
   ...
   delete from BudgetCategoryFinal where BudgetID=@BudgetID and Budgetfileid=@BudgetFileID;
   delete from BudgetAccountsFinal where BudgetFileID=@BudgetFileID and BudgetID=@BudgetID;
     ...
     DEALLOCATE Cus_Category1
	  ...
	  ------end------------------------      
	   ...
	   exec InsertDataETCEFC @BudgetID,@BudgetFileID
	    ...
	    update BudgetFile set Status='Saved' where Budgetid=@BudgetID;
