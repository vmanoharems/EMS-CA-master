...
GO
Create FUNCTION [dbo].[convertOptionalcodes] (@stringdata varchar(400))
RETURNS varchar(400)
AS
BEGIN 
...
select @transcode=SegmentCode  from Segment  where SegmentLevel=@transcodeid
select @transvalue=AccountCode  from TblAccounts  where AccountId=@transvalueid
