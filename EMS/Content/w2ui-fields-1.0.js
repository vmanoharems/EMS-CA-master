	...
	w2field.prototype = {
			...
			if (options.autoFormat && val != '') {
				switch (this.type) {
					...
					case 'percent':
						...
						break;
					case 'float':
					    val = w2utils.formatNumber(options.precision ? Number(val).toFixed(options.precision) : Number(val).toFixed(2), options.groupSymbol);
						break;
					case 'int':
		...
		keyDown: function (event, extra) {
			...
			if (['int', 'float', 'money', 'currency', 'percent'].indexOf(this.type) != -1) {
				...
				switch (key) {
				    case 38: // up
                        // By sanjay key up and down to (+ and -) amount stoped
