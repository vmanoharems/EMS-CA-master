...
var APIUrlGetGetInvoicePDF = HOST + "/api/ReportAPI/GetInvoiceListPosted";
...
function GetInvoiceListSucess(response)
{
    ...
    for (var i = 0; i < Tcount; i++) {
        ...
        strhtml += '<td><a href="#" style="color: #337ab7;">' + response[i].InvoiceNumber + '</a></td>';
...
function GetInvoicePDF() {
    alert('ACS');
    $.ajax({
        url: APIUrlGetGetInvoicePDF + '?ProdId=' + localStorage.ProdId,
        cache: false,
        beforeSend: function (request) {
            request.setRequestHeader("Authorization", localStorage.EMSKeyToken);
        },
